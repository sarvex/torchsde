...
class SRK(base_solver.BaseSDESolver):
    ...
    def __init__(self, sde, **kwargs):
        ...
        if isinstance(sde, adjoint_sde.AdjointSDE):
            raise ValueError(
                'Stochastic Runge–Kutta methods cannot be used for adjoint SDEs, because it requires direct access to the diffusion, whilst adjoint SDEs rely on a more efficient diffusion-vector product. Use a different method instead.'
            )
